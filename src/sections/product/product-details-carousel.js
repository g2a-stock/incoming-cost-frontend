...
function upscaleUrlImage(url) {
  if (!url)
    return 'https://images.g2a.com/g2a/690x1008/0x1x1/ad6434749f58/5912fb37ae653ae5805b4842';
  // Разделяем URL на части
  const parts = url.split('/');

  // Находим часть URL, содержащую размеры (предполагаем, что это 230x336)
  const sizePart = parts.find((part) => /\d+x\d+/.test(part));

  if (sizePart) {
    // Разделяем размеры на ширину и высоту
    const [width, height] = sizePart.split('x').map(Number);

    // Увеличиваем размеры в два раза
    const newWidth = width * 3;
    const newHeight = height * 3;

    // Заменяем старые размеры на новые в URL
    return url.replace(sizePart, `${newWidth}x${newHeight}`);
  }

  // Возвращаем исходный URL, если размеры не найдены
  return url;
}
...
export default function ProductDetailsCarousel({ product }) {
  ...
  const slides = [
    {
      src: upscaleUrlImage(product?.smallImage),
    },
  ];
